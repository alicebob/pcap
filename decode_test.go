...
func TestDecodeLinuxCooked(t *testing.T) {
	...
	p := &Packet{
		DatalinkType: LinkTypeLinuxCooked,
		Data: []byte{
...
func TestDecodeNFLog(t *testing.T) {
	// NFLog packet. and a UDP packet.
	p := &Packet{
		DatalinkType: LinkTypeNFLog,
		Data: []byte{
			,
		},
		Len: 128,
	}
	p.Decode()
	...
	if tcp.SrcPort != 49565 {
		t.Error("tcp srcport", tcp.SrcPort)
	}
	if tcp.DestPort != 80 {
		t.Error("tcp destport", tcp.DestPort)
	}
	if tcp.PayloadLength != 6 {
		t.Error("tcp PayloadLength", tcp.PayloadLength)
	}

	// Leftover payload (so this is TCP payload)
	if len(p.Payload) != 6 {
		t.Error("Wrong payload length")
	}
}
